...
8. **Salvar em PDF no tom C:**
```bash
python cifra_standalone.py coldplay the-scientist C --pdf
```

9. **Salvar e abrir PDF automaticamente:**
```bash
python cifra_standalone.py coldplay the-scientist C --abrir
```

10. **Salvar em PDF no tom original:**
```bash
python cifra_standalone.py coldplay the-scientist --pdf
```
...
python cifra_standalone.py projeto-sola isaias-53 Am --abrir
```
...
Adicione `--pdf` ao final do comando para salvar a cifra em PDF:

```bash
# Salvar no tom original
python cifra_standalone.py coldplay the-scientist --pdf

# Salvar transposto para C
python cifra_standalone.py coldplay the-scientist C --pdf

# Salvar transposto +2 semitons
python cifra_standalone.py coldplay the-scientist 2 --pdf
```

### Modo Interativo

1. Busque e visualize a cifra
2. Transponha para o tom desejado (opcional)
3. Digite `pdf` para salvar
4. O PDF será criado com o tom atual

### Características dos PDFs

- ✅ **Layout em duas colunas** para melhor aproveitamento da página
- ✅ **Formatação profissional** com fonte monoespaçada para acordes
- ✅ **Cabeçalho e rodapé** em todas as páginas com informações da música
- ✅ **Preserva acordes com letras** - não quebra cifras entre linhas
- ✅ **Quebra inteligente de páginas** - seções completas não são divididas
- ✅ **Nome automático**: `Artista_Musica_Tom.pdf`
- ✅ **Inclui metadados**: tom original, transposição, YouTube, fonte
- ✅ **Pasta organizada**: todos os PDFs ficam em `/pdf`
- ✅ **Criação automática**: a pasta `/pdf` é criada se não existir
- ✅ **Abertura automática**: use `--abrir` para abrir o PDF após salvar

**Exemplo de nome de arquivo:**
- `Coldplay_The_Scientist_C.pdf` - transposto para C
- `Coldplay_The_Scientist_F.pdf` - tom original F
...
- ✅ **Salva cifras em PDF** com o tom selecionado
...
8. **Use `--pdf` para salvar**: os PDFs são salvos automaticamente na pasta `/pdf`
9. **Nomes de arquivo PDF**: formato `Artista_Musica_Tom.pdf`
...
| `pdf/`                       | Pasta onde os PDFs são salvos automaticamente      |
...
- **Salvamento em PDF**: Use `--pdf` no standalone ou digite `pdf` no interativo
...
Opções:
  ...
  [0] - Ver tom original
  [pdf] - Salvar a cifra atual em PDF
  [s] - Sair
======================================================================
